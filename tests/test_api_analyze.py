from fastapi.testclient import TestClient

from analysis.main import app
from analysis.models.schemas import SuspicionLevel
from analysis.routers import analyze as analyze_router


def _post_text(client: TestClient, text: str):
    return client.post(
        "/analyze/text",
        json={
            "input_type": "text",
            "content": text,
        },
    )


def test_health_endpoint_reports_model_status():
    with TestClient(app) as client:
        response = client.get("/health")

    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "ok"
    assert "kobert_ready" in payload
    assert "ready" in payload
    assert "warmup_in_progress" in payload


def test_ready_endpoint_reports_server_is_ready_after_startup():
    with TestClient(app) as client:
        response = client.get("/ready")

    assert response.status_code == 200
    payload = response.json()
    assert payload["status"] == "ok"
    assert payload["ready"] is True


def test_analyze_text_flags_medical_claim():
    with TestClient(app) as client:
        response = _post_text(client, "???¬ë¦¼?€ ?„í† ?¼ë? ì¹˜ë£Œ?©ë‹ˆ??")

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "?˜ì‹¬"
    assert payload["sentence_results"][0]["suspicion_level"] == "?˜ì‹¬"


def test_analyze_text_flags_exaggerated_claim():
    with TestClient(app) as client:
        response = _post_text(client, "??7??ë§Œì— ê¸°ë? ?„ì „ ?œê±°, 100% ?¨ê³¼ ë³´ì¥.")

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "?˜ì‹¬"
    assert payload["sentence_results"][0]["suspicion_level"] == "?˜ì‹¬"


def test_analyze_text_keeps_allowed_moisturizing_claim_normal():
    with TestClient(app) as client:
        response = _post_text(client, "?¼ë? ë³´ìŠµ???„ì???ì£¼ëŠ” ?¬ë¦¼?…ë‹ˆ??")

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "?•ìƒ"
    assert payload["sentence_results"][0]["suspicion_level"] == "?•ìƒ"


def test_analyze_text_does_not_whitelist_doctor_recommendation_claim():
    with TestClient(app) as client:
        response = _post_text(client, "?˜ì‚¬ê°€ ì¶”ì²œ?˜ëŠ” ?¸ëŸ¼?¼ë¡œ ì£¼ë¦„??ë°˜ë“œ???¬ë¼ì§‘ë‹ˆ??")

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "?˜ì‹¬"
    assert payload["sentence_results"][0]["suspicion_level"] == "?˜ì‹¬"


def test_analyze_text_flags_100_percent_variant_claim():
    with TestClient(app) as client:
        response = _post_text(client, "\uD53C\uBD80 \uAC01\uC9C8 100\uD504\uB85C \uC81C\uAC70")

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "\uC758\uC2EC"
    assert payload["sentence_results"][0]["suspicion_level"] == "\uC758\uC2EC"


def test_analyze_text_keeps_caution_for_all_solved_claim():
    with TestClient(app) as client:
        response = _post_text(client, "\uD53C\uBD80 \uBCF4\uC2B5 \uAC74\uC870 \uBAA8\uB450 \uD574\uACB0")

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "\uC8FC\uC758"
    assert payload["sentence_results"][0]["suspicion_level"] == "\uC8FC\uC758"


def test_analyze_image_uses_ocr_text(monkeypatch):
    monkeypatch.setattr(analyze_router, "extract_from_image", lambda _: "??7??ë§Œì— ê¸°ë? ?„ì „ ?œê±°, 100% ?¨ê³¼ ë³´ì¥.")

    with TestClient(app) as client:
        response = client.post(
            "/analyze/image",
            files={"file": ("sample.png", b"fake-image", "image/png")},
        )

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "?˜ì‹¬"
    assert payload["sentence_results"][0]["suspicion_level"] == "?˜ì‹¬"


def test_analyze_image_rejects_non_image_file():
    with TestClient(app) as client:
        response = client.post(
            "/analyze/image",
            files={"file": ("sample.txt", b"not-image", "text/plain")},
        )

    assert response.status_code == 400
    assert response.json()["detail"] == "?´ë?ì§€ ?Œì¼ë§??…ë¡œ??ê°€?¥í•©?ˆë‹¤."


def test_analyze_text_does_not_drop_sentence_only_because_ad_filter_is_negative(monkeypatch):
    monkeypatch.setattr(analyze_router, "predict_ad", lambda _: (False, 0.02))
    monkeypatch.setattr(analyze_router, "predict_cosmetic", lambda _: (False, 0.01))

    with TestClient(app) as client:
        response = _post_text(client, "???¬ë¦¼?€ ?„í† ?¼ë? ì¹˜ë£Œ?©ë‹ˆ??")

    assert response.status_code == 200
    payload = response.json()
    assert payload["overall_suspicion_level"] == "?˜ì‹¬"
    assert payload["sentence_results"][0]["suspicion_level"] == "?˜ì‹¬"
    assert "ê±´ë„ˆ?€" not in payload["sentence_results"][0]["reason"]


def test_analyze_text_forwards_uncertain_domain_copy_to_kobert(monkeypatch):
    monkeypatch.setattr(analyze_router, "predict_ad", lambda _: (False, 0.02))
    monkeypatch.setattr(analyze_router, "predict_cosmetic", lambda _: (False, 0.01))

    async def fake_analyze_with_kobert(sentence, rule_result):
        assert sentence == "ê¸‰ì´ ?¤ë¥¸ 98% ë§ˆë°ì¹´ì†Œ?¬ì´??
        assert rule_result.suspicion_level.value == "?•ìƒ"
        return rule_result.model_copy(
            update={
                "suspicion_level": SuspicionLevel.CAUTION,
                "reason": "AIê°€ ì¶”ê? ?ë‹¨?ˆìŠµ?ˆë‹¤.",
                "score": 0.42,
            }
        )

    monkeypatch.setattr(analyze_router, "analyze_with_kobert", fake_analyze_with_kobert)

    with TestClient(app) as client:
        response = _post_text(client, "ê¸‰ì´ ?¤ë¥¸ 98% ë§ˆë°ì¹´ì†Œ?¬ì´??)

    assert response.status_code == 200
    payload = response.json()
    assert payload["sentence_results"][0]["suspicion_level"] == "ì£¼ì˜"
    assert payload["sentence_results"][0]["reason"] == "AIê°€ ì¶”ê? ?ë‹¨?ˆìŠµ?ˆë‹¤."

