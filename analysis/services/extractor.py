...
def _normalize_ocr_text(text: str) -> str:
    ...
    cleaned = re.sub(r"([ê°€-?£A-Za-z0-9])([,.:;!?])", r"\1\2 ", cleaned)
    ...
    cleaned = re.sub(r"(?<=[ê°€-??)\s*/\s*(?=[ê°€-??)", "/", cleaned)
    ...
    cleaned = re.sub(r"?¸ì²´ ?ìš© ?œí—˜ ?„ë£Œ", "?¸ì²´?ìš©?œí—˜ ?„ë£Œ", cleaned)
...
def _ocr_quality_score(text: str, confidences: list[float]) -> float:
    ...
    text_len = len(text)
    korean_chars = sum("ê°€" <= ch <= "?? for ch in text)
...
def _extract_text_rows(image_bytes: bytes) -> list[bytes]:
    """
    ì²´í¬ë¦¬ìŠ¤??ë°°ë„ˆ???´ë?ì§€?ì„œ ?ìŠ¤??ì¤??¨ìœ„ë¡?ë¶„ë¦¬?œë‹¤.
    """
    rows: list[bytes] = []
    try:
        img = _decode_image(image_bytes)
...
def _build_ocr_variants(image_bytes: bytes) -> list[tuple[str, bytes]]:
    """
    ?ë³¸ + ?¬ëŸ¬ ?„ì²˜ë¦?ë²„ì „??ë§Œë“¤??OCR ?±ê³µë¥ ì„ ?’ì¸??
    """
...
def _parse_google_vision_response(payload: dict) -> dict:
    ...
    if error:
        message = error.get("message") or "Google Vision OCR ?”ì²­???¤íŒ¨?ˆìŠµ?ˆë‹¤."
...
def _run_google_vision_once(image_bytes: bytes) -> dict:
    if not GOOGLE_VISION_API_KEY:
        raise RuntimeError("GOOGLE_VISION_API_KEYê°€ ?¤ì •?˜ì–´ ?ˆì? ?ŠìŠµ?ˆë‹¤.")
    ...
    try:
        ...
        raise RuntimeError(f"Google Vision OCR HTTP ?¤ë¥˜({e.response.status_code}): {e.response.text}") from e
    except httpx.HTTPError as e:
        raise RuntimeError(f"Google Vision OCR ?¸ì¶œ ?¤íŒ¨: {e}") from e

    try:
        payload = response.json()
    except ValueError as e:
        raise RuntimeError("Google Vision OCR ?‘ë‹µ??JSON???„ë‹™?ˆë‹¤.") from e
...
def extract_from_image(image_bytes: bytes) -> str:
    """
    ?´ë?ì§€?ì„œ ?ìŠ¤??ì¶”ì¶œ.
    - ?¬ëŸ¬ ?„ì²˜ë¦?ë²„ì „??ë§Œë“  ??EasyOCRë¡??ìˆ˜??    - 1ì°?ê²°ê³¼ ?ˆì§ˆ????œ¼ë©??„í™”??thresholdë¡?fallback ?¬ì‹œ??    - ê²°ê³¼??ê°„ë‹¨??OCR ?¤í? êµì •??ê±°ì³ ë°˜í™˜
    """
    engine = OCR_ENGINE
    ...
    if engine in {"google", "vision", "google_vision"}:
        try:
            ...
            raise RuntimeError(f"Google Vision OCR ?ìŠ¤??ì¶”ì¶œ ?¤íŒ¨: {e}")

    if engine == "paddle":
        if not _PADDLEOCR_AVAILABLE:
            raise RuntimeError("PaddleOCRê°€ ?¤ì¹˜?˜ì–´ ?ˆì? ?ŠìŠµ?ˆë‹¤. pip install paddleocr ë¥??¤í–‰?˜ì„¸??")
        try:
            return _extract_with_paddleocr_variants(image_bytes)
        except Exception as e:
            raise RuntimeError(f"PaddleOCR ?ìŠ¤??ì¶”ì¶œ ?¤íŒ¨: {e}")

    if engine == "easy":
        if not _EASYOCR_AVAILABLE:
            raise RuntimeError("EasyOCRê°€ ?¤ì¹˜?˜ì–´ ?ˆì? ?ŠìŠµ?ˆë‹¤. pip install easyocr ë¥??¤í–‰?˜ì„¸??")
        try:
            return _extract_with_easyocr_variants(image_bytes)
        except Exception as e:
            raise RuntimeError(f"EasyOCR ?ìŠ¤??ì¶”ì¶œ ?¤íŒ¨: {e}")
    ...
    if not _EASYOCR_AVAILABLE:
        raise RuntimeError("?¬ìš© ê°€?¥í•œ OCR ?”ì§„???†ìŠµ?ˆë‹¤. paddleocr ?ëŠ” easyocrë¥??¤ì¹˜?˜ì„¸??")
    try:
        return _extract_with_easyocr_variants(image_bytes)
    except Exception as e:
        raise RuntimeError(f"EasyOCR ?ìŠ¤??ì¶”ì¶œ ?¤íŒ¨: {e}")
...
def split_sentences(text: str) -> list[str]:
    """?ìŠ¤?¸ë? ë¬¸ì¥ ?¨ìœ„ë¡?ë¶„ë¦¬ (kss ?°ì„ , ?†ìœ¼ë©??•ê·œ???´ë°±)"""
    if _KSS_AVAILABLE:
...
def _split_with_kss(text: str) -> list[str]:
    """kss ?¼ì´ë¸ŒëŸ¬ë¦¬ë? ?¬ìš©???œêµ­??ë¬¸ì¥ ë¶„ë¦¬"""
    try:
...
_SENTENCE_END_RE = re.compile(
    r"([.!??‚ï¼ï¼?$|"
    r"(?µë‹ˆ???©ë‹ˆ???©ë‹ˆ???ˆìŠµ?ˆë‹¤|?œë¦½?ˆë‹¤|?…ë‹ˆ???©ë‹ˆ???˜ì„¸???¼ìš”|?´ìš”|?˜ìš”|?´ìš”|?„ìš”|????$)"
)
...
def _is_short_ocr_fragment(text: str) -> bool:
    """
    OCR ê´‘ê³  ?´ë?ì§€??ì¹´í”¼ê°€ ì¤??¨ìœ„ë¡??Šê²¨ ?¤ì–´?¤ëŠ” ê²½ìš°ê°€ ë§ë‹¤.
    ?„ê²° ë¬¸ì¥???„ë‹Œ ì§§ì? ì¤„ì? ?¤ìŒ ì¤„ê³¼ ?©ì³ ë¬¸ë§¥??ë³´ì¡´?œë‹¤.
    """
    s = text.strip()
...
def _merge_sentence_fragments(sentences: list[str]) -> list[str]:
    """
    ì¤„ë°”ê¿?ê¸°ë°˜ OCR ì¡°ê°???˜ë? ?¨ìœ„ë¡??¬ê²°?©í•œ??
    ?? "ê¸°ë??¡í‹°\\n?„ë ¥?€??\n...\\në¦¬ì…‹?˜ì„¸?? -> ??ì¹´í”¼ ë¬¸ì¥
    """
    merged: list[str] = []
    buffer: list[str] = []
