...
from analysis.services.extractor import extract_from_image, split_sentences
...
async def analyze_default(request: AnalyzeRequest):
    ...
    return await _run_analysis(request.content, source_type=request.input_type.value)
        ...
        if is_noise(sentence):
            ...
            rule_results.append(
                SentenceResult(
                    ...
                    matched_keywords=[],
                    matched_patterns=[],
                    reason="의미 없는/잡음성 문장으로 판단되어 건너뜀.",
                    score=0.0,
                )
            )
            continue
